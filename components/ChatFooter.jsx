  ...
  return (
    <div className='flex items-center bg-c1/[0.5] p-2 rounded-xl relative'>
        ...
        <div className='shrink-0'>
            <input type="file" id='fileUploader' className='hidden' onChange={onFileChange}/>
            <label htmlFor='fileUploader'>
                <Icon size="large" icon={<CgAttachment size={20} className='text-c3'/>}/>
            </label>
        </div>
        <div className='shrink-0 relative'>
            <Icon size="large" className={``} icon={<HiOutlineEmojiHappy size={24} className='text-c3'/>} onClick={()=>setShowImojiPicker(true)}/>
            {showImojiPicker &&
            <ClickAwayListener onClickAway={()=>setShowImojiPicker(false)}>
            <div className='absolute bottom-12 left-0 shadow-lg'>
                <EmojiPicker emojiStyle='twitter' theme='dark' onEmojiClick={onEmojiClick} autoFocusSearch={false}/>
            </div>
            </ClickAwayListener>
             }
        ...
        {editMsg && <div className='absolute -top-12 left-1/2 -translate-x-1/2 bg-c4 flex items-center gap-2 py-2 px-4 pr-2 rounded-full text-sm font-semibold cursor-pointer shadow-lg' onClick={()=>setEditMsg(null)}>
